...
function pad(e, t) { for (var a = e + ""; a.length < t;) a = "0" + a; return a }
...
function CoundownTimer(e) {
    var t = 60 * e;
    myInterval = setInterval((function() { myTimeSpan = 1e3 * t, $(".timer-title").text(GetTime(myTimeSpan)), t < 600 ? ($(".timer-title").addClass("time-ending"), $(".timer-title").removeClass("time-started")) : ($(".timer-title").addClass("time-started"), $(".timer-title").removeClass("time-ending")), t > 0 ? t -= 1 : CleartTimer() }), 1e3)
}

function CleartTimer() { clearInterval(myInterval), $("title").text("Time Out"), $("#btnYesSubmitConfirm").trigger("click") }
...
function pretty_time_string(e) { return (e < 10 ? "0" : "") + e }

function CheckQueExists(e) { $.each(AttemptedAns, (function(t, a) { void 0 !== a && a[1] == e && AttemptedAns.splice(t, 1) })) }
...
function CheckResult() {
    ...
    })), $("#lblRTotalQuestion").text(TotalQuestion), $("#lblRTotalAttempted").text(TotalAttempted), $("#lblRTotalCorrect").text(TotalCorrect), $("#lblRTotalWrong").text(TotalWrong), $("#lblRScore").text(score), $("#theadres").DataTable({ paging: !0, sorting: !1 })
}
$(document).ready((function() {
    "14" == $("#ExamID").val() && $("#drplanguage").append($("<option></option>").text("Gujarati").val("gujarati")), $("#page01").show(), $(".exam-paper").show(), CoundownTimer(parseInt($("#hdfTestDuration").val())), CheckNextPrevButtons(), CheckQueAttemptStatus(), $("#drpQuestionsGroup").on("change", (function() { $(".test-questions").find("li:nth-child(" + $(this).val() + ")").find("a").trigger("click") })), $("#btnPrevQue").click((function() { PrevQuestion(!0) })), $("#btnNextQue").click((function() { NextQuestion(!0) })), $(".test-ques").click((function() {
        ...
        if ($("input[name='radios" + a + "']").each((function() { $(this).is(":checked") && (n = !0) })), 0 == n) return alert("Please choose an option"), !1;
        ...
        var t = $(".test-questions").find("li.active"),
            a = t.find("a").attr("data-href");
        $("#" + a).attr("data-queid"), t.find("a").removeClass("saved-que"), $("input[name='radios" + a + "']:checked").each((function() { $(this).prop("checked", !1).change() })), $("input[name='chk" + a + "']").each((function() { $(this).prop("checked", !1).change() })), $("input[type=checkbox]").prop("checked", !1).change(), $("input[type=text]").val(""), a = t.find("a").attr("data-href"), $("#" + a).find(".hdfQuestionID").val(), $("#" + a).find(".hdfPaperSetID").val(), $("#" + a).find(".hdfCurrectAns").val(), $("#" + a).find(".hdfCurrectAns").val(), t.find("a").removeClass("que-save-mark"), t.find("a").removeClass("que-mark"), t.find("a").removeClass("que-save"), t.find("a").removeClass("que-not-attempted"), t.find("a").addClass("que-not-answered"), CheckQueAttemptStatus()
    ...
    })), $("#btnYesSubmitConfirm").on("click", (function(e) { e.preventDefault(), $(".exam-thankyou").show(), $(".exam-paper").hide(), $("#divdrplngcng").hide(), $(".exam-summery").hide() })), $("#btnNoSubmitConfirm").on("click", (function(e) { e.preventDefault(), $(".exam-paper").show(), $(".stream_1").show(), $(".exam-confirm").hide(), $("#divdrplngcng").show(), $(".exam-summery").hide() })), $(".drplanguage").on("change", (function(e) {
        e.preventDefault();
        ...
        $(".question-height > .MQ").each((function(index, item) {
            ...
            $.each(JSON.parse(PaperQuestions), (function(i, item) { item.QuestionNo == currentQuestionNo && item.LangID == parseInt(newlang) && (currentDisImg.attr("src", $("#datalink").val() + item.DescriptionImagePath), currentImg.attr("src", $("#datalink").val() + item.QuestionImagePath)) }))
    ...
    })), $("#btnViewResult").on("click", (function(e) { e.preventDefault(), CheckResult(), $(".exam-result").show(), $(".exam-thankyou").hide(), $("#divdrplngcng").hide() })), $("#btnRBack").on("click", (function(e) { e.preventDefault(), window.location.href = $("#hdfBaseURL").val() + "Quiz/Home/Index" })), $(".full_screen").click((function() { $("#quest").removeClass("col-md-8"), $("#quest").addClass("col-md-12"), $("#pallette").addClass("hidden"), $(".full_screen").addClass("hidden"), $(".collapse_screen").removeClass("hidden") })), $(".collapse_screen").click((function() { $("#quest").removeClass("col-md-12"), $("#quest").addClass("col-md-8"), $("#pallette").removeClass("hidden"), $(".full_screen").removeClass("hidden"), $(".collapse_screen").addClass("hidden") }))
}));